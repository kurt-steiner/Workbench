...
class _TaskAddState extends State<TaskAdd> with StateMixin {
  late final PostTaskRequest request;
  final TextEditingController nameController = TextEditingController(text: "");
  final TextEditingController encouragementController = TextEditingController(text: "");
  final TextEditingController wordController = TextEditingController(text: "");
  final modeNotifier = ValueNotifier(da.IconMode.word);
  final imageNotifier = ValueNotifier<da.ImageCompose>(da.ImageCompose());
  ...
  void initState() {
    ...
    request = PostTaskRequest(
        name: "",
        icon: da.Word(char: "文", color: FlatUIColor.Orange),
        encouragement: "",
        frequency: da.Days(weekdays: []),
        goal: da.CurrentDay(),
        keepdays: da.Forever(),
        group: da.Group.noon,
  ...
  Widget build(BuildContext context) {
    ...
    return step == 1 ? buildStep1(context) : buildStep2(context);
  }
    ...
    return Scaffold(
              ...
              builder: (_, value, child) => TextButton(
                onPressed: value ? null : () {
                  if (modeNotifier.value == da.IconMode.image) {
                    request.icon = da.Image(entryId: entryId, backGroundId: backGroundId, backGroundColor: backGroundColor);
                  } else {
                    request.icon = da.Word(char: wordController.text, color: backGroundColor);
                  }

                  request.name = nameController.text;
                  request.encouragement = encouragementController.text;
      ...
      body: ListView(
        children: [
          ...
          Card(child: buildSelectColor(context)),
  ...
  Widget buildStep2(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        children: [
          TaskEditFrequency(postRequest: request),

          Card(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                buildEditGoal(context),
                buildEditStartTime(context),
                buildEditKeepDays(context)
              ],
            ),
          ),
  ...
  Widget buildEditGoal(BuildContext context) {
    return StatefulBuilder(
      builder: (context, setState) {
        ...
        if (request.goal is da.CurrentDay) {
          text = "当天完成一次";
        } else {
          da.Amount amount = request.goal as da.Amount;
  ...
  Widget buildEditGoalContent(BuildContext context, void Function(da.Goal) onChanged) {
    ...
    String unit = TaskAdd.units.first;
              ...
              children: [
                SizedBox(
                  ...
                  child: TextField(
                    ...
                    onChanged: (value) {
                      if (value.isNotEmpty && TaskAdd.postiveRegex.hasMatch(value)) {
                        setState(() {
                          textTotal = value;
                          resultGoal = da.Amount(total: int.parse(textTotal), unit: unit, eachAmount: int.parse(textEachAmount));
                          onChanged(resultGoal);
    ...
    return StatefulBuilder(
      builder: (context, setState) => ListTile(
        ...
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text(request.keepdays.toString()),
            const SizedBox(width: 4,),
            const Icon(Icons.arrow_forward_ios_rounded)
          ],
        ),
  ...
  Future<da.KeepDays?> showDialogEditKeepDays(BuildContext context) async {
    da.KeepDays keepdays = request.keepdays.copy();
      ...
      builder: (context, setState) {
        final Widget addButton = TextButton(
          onPressed: () async {
            ...
            if (timeOfDay != null) {
              setState(() {
                request.notifyTimes.add(da.NotifyTime(hour: timeOfDay.hour, minute: timeOfDay.minute));
              });
            }
          },
        ...
        final List<Widget> children = [
          ...request.notifyTimes.map<Widget>((e) => buildNotifyTimeItem(context, time: e, setState: setState)).toList(),
          addButton
        ];

        return ListTile(
          title: const Text("提醒"),
          subtitle: request.notifyTimes.isEmpty ? addButton : Wrap(
            crossAxisAlignment: WrapCrossAlignment.center,
            children: children
          ),
        );
  ...
  Widget buildNotifyTimeItem(BuildContext context, {required da.NotifyTime time, required void Function(void Function()) setState}) {
    return InputChip(
      ...
      onPressed: () async {
        ...
        if (timeOfDay != null) {
          setState(() {
            int index = request.notifyTimes.indexWhere((element) => element == time);
